    ...
    */
    public sealed class IRtcEngine : IRtcEngineNative
    {
        ...
        private IRtcEngine(string appId)
        {
            InitGameObject();
            InitEngineCallback();
            int initSuccess = IRtcEngineNative.createEngine(appId);
            if (initSuccess != 0) {
                Debug.LogError("Create engine failed, error code: " + initSuccess);
            } 
        ...
        private IRtcEngine(RtcEngineConfig engineConfig)
        {
            InitGameObject();
            InitEngineCallback();
            int initSuccess = IRtcEngineNative.createEngine2(engineConfig.appId, (uint)engineConfig.areaCode);
            if (initSuccess != 0) {
                Debug.LogError("Create engine failed, error code: " + initSuccess);
            } 
            mAudioEffectM = AudioEffectManagerImpl.GetInstance(this);
         ...
         * 
         * @note
         * - When `mute` is set as true, this method does not disable the microphone, which does not affect any ongoing capturing.
         * - You can call this method either before or after joining a channel. If you call {@link agora_gaming_rtc.IRtcEngine.SetChannelProfile SetChannelProfile} after this method, the SDK resets whether or not to mute the local audio according to the channel profile and user role. Therefore, we recommend calling this method after the `SetChannelProfile` method.
         *
         ...
         * - If you called the {@link agora_gaming_rtc.IRtcEngine.MuteAllRemoteAudioStreams MuteAllRemoteAudioStreams} method 
         * and set `mute` as `true` to stop receiving all remote users' audio streams, call the `MuteAllRemoteAudioStreams` method 
         * and set `mute` as `false` before calling this method. The `MuteAllRemoteAudioStreams` method sets all remote audio streams, 
         * while the `MuteRemoteAudioStream` method sets a specified remote audio stream.
         *
        ...
        * - You can call this method either before or after joining a channel. If you call {@link agora_gaming_rtc.IRtcEngine.SetChannelProfile SetChannelProfile} after this method, the SDK resets whether or not to mute the local video according to the channel profile and user role. Therefore, we recommend calling this method after the `SetChannelProfile` method.
        * 
        ...
        * - If you called the {@link agora_gaming_rtc.IRtcEngine.MuteAllRemoteVideoStreams MuteAllRemoteVideoStreams} method and 
        ...
        */
         ...
         * - Agora supports four encryption modes. For Android and iOS, if you choose an encryption mode (excepting `SM4_128_ECB` mode), you need to add an external encryption library.
         *
        ...
        {
            return IRtcEngineNative.enableEncryption(enabled, encryptionConfig.encryptionKey, (int)encryptionConfig.encryptionMode);
        }
        ...
        private static void OnRemoteAudioStatsCallback(uint uid, int quality, int networkTransportDelay, int jitterBufferDelay, int audioLossRate, int numChannels, int receivedSampleRate, int receivedBitrate, int totalFrozenTime, int frozenRate, int totalActiveTime, int publishDuration)
        {
